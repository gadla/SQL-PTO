...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- Windows Version and Architecture subsection'
--------------------------------------------------------------------------------------------------------------------------------
...
BEGIN
	SET @sqlcmd = N'SELECT @winverOUT = CASE WHEN windows_release IN (''6.3'',''10.0'') AND @@VERSION LIKE ''%Build 10586%'' THEN ''10.0'' ELSE windows_release END, @winspOUT = windows_service_pack_level, @archOUT = CASE WHEN @@VERSION LIKE ''%<X64>%'' THEN 64 WHEN @@VERSION LIKE ''%<IA64>%'' THEN 128 ELSE 32 END FROM sys.dm_os_windows_info (NOLOCK)';
...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- HA Information subsection'
--------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- Linked servers info subsection'
--------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- Instance info subsection'
--------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- Buffer Pool Extension info subsection'
--------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- Resource Governor info subsection'
--------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- Logon triggers subsection'
--------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- Database Information subsection'
--------------------------------------------------------------------------------------------------------------------------------
...
BEGIN
	...
	db.[compatibility_level] AS [DB_Compatibility_Level], db.collation_name AS [DB_Collation], 
	db.page_verify_option_desc AS [Page_Verify_Option], db.is_auto_create_stats_on, db.is_auto_update_stats_on,
	db.is_auto_update_stats_async_on, db.delayed_durability_desc AS [delayed_durability_status], db.is_parameterization_forced, 
	db.snapshot_isolation_state_desc, db.is_read_committed_snapshot_on,
	db.is_read_only, db.is_auto_close_on, db.is_auto_shrink_on,
	CASE WHEN db.target_recovery_time_in_seconds > 0 THEN 1 ELSE 0 END AS is_indirect_checkpoint_on,
...
BEGIN
	...
	db.is_parameterization_forced, db.is_query_store_on, db.snapshot_isolation_state_desc, db.is_read_committed_snapshot_on,
	db.is_read_only, db.is_auto_close_on, db.is_auto_shrink_on, 
	CASE WHEN db.target_recovery_time_in_seconds > 0 THEN 1 ELSE 0 END AS is_indirect_checkpoint_on,
	db.target_recovery_time_in_seconds, db.is_encrypted, db.is_trustworthy_on, db.is_db_chaining_on, db.is_parameterization_forced, 
...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- Database file autogrows last 72h subsection'
--------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- Database triggers subsection'
--------------------------------------------------------------------------------------------------------------------------------
IF @ptochecks = 1
BEGIN
...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- Enterprise features usage subsection'
--------------------------------------------------------------------------------------------------------------------------------
IF @sqlmajorver > 9
BEGIN
...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- Backups since last Full Information subsection'
--------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- System Configuration subsection'
--------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- Processor Affinity in NUMA architecture subsection'
--------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- Additional Processor information subsection'
--------------------------------------------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------------------------------------
PRINT '-- Server Memory subsection'
--------------------------------------------------------------------------------------------------------------------------------
...
DECLARE @is_db_chaining_on bit--, @is_indirect_checkpoint_on bit, @target_recovery_time_in_seconds int
...
BEGIN
	...
	--0 AS is_indirect_checkpoint_on, 0 AS target_recovery_time_in_seconds, 
...
SELECT @is_auto_close_on = 0, @is_auto_shrink_on = 0, @page_verify_option = 0, @is_auto_create_stats_on = 0, @is_auto_update_stats_on = 0, @is_db_chaining_on = 0, @is_trustworthy_on = 0, @is_parameterization_forced = 0--, @is_indirect_checkpoint_on = 0, @target_recovery_time_in_seconds  = 0
...
BEGIN
	...
	WHERE is_auto_close_on = 1 OR is_auto_shrink_on = 1 OR page_verify_option <> 2 OR is_db_chaining_on = 1 OR is_auto_create_stats_on = 0 
		OR is_auto_update_stats_on = 0 OR is_trustworthy_on = 1 OR is_parameterization_forced = 1 --OR is_indirect_checkpoint_on = 1;
END
ELSE
BEGIN
...
foreach($vol in $vols)
{
	...
	Write-Output $drive
				...
				FROM @output_hw_tot WHERE [PS_OUTPUT] LIKE '%cannot be loaded because%'
					...
					OR [PS_OUTPUT] LIKE '%scripts est d�sactiv�e%') = 0
				BEGIN
...
',16,1);
		...
		BEGIN
			...
			IF @ctr4 > 0
			BEGIN
				PRINT '-- Backup_and_Database_locations'
...
BEGIN
	...
	(
		...
		SELECT  waiting_task_address, session_id, exec_context_id, wait_duration_ms, 
			...
			CASE WHEN [wait_type] LIKE 'PAGE%' AND [resource_description] LIKE '%:%' THEN CAST(RIGHT([resource_description], LEN([resource_description]) - CHARINDEX(':', [resource_description], LEN([resource_description])-CHARINDEX(':', REVERSE([resource_description])))) AS bigint)
