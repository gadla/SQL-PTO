...
where name = 'system_health' and t.target_name = 'event_file'
).value('(./EventFileTarget/File/@name)[1]', 'varchar(2000)')

select @path_to_health_session = left(@path_to_health_session, charindex('system_health', @path_to_health_session)-1)
select @path_to_health_session = @path_to_health_session + 'system_health*.xel'

select LTRIM(RTRIM(@path_to_health_session))
